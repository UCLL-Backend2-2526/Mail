...
We gebruiken Mailpit om dit uit te testen. Mailpit is een SMTP-server die je toelaat om alle verzonden mails te zien.
De mails worden niet echt verzonden. In `compose.yaml` zie je hoe een mailpit container geconfigureerd wordt. Als je
dit start met Docker Compose kan je alle mails zien op http://localhost:8025.

Meer info over `JavaMailSender`:

- [Spring Reference documentatie van Email](https://docs.spring.io/spring-framework/reference/6.2/integration/email.html)
